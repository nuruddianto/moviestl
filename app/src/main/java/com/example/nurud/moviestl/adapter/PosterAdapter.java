...
public class PosterAdapter extends RecyclerView.Adapter<PosterAdapter.ViewHolder> {
    ...
    public ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        viewHolder.movieImage = (ImageView) cardView.findViewById(R.id.movie_image);
    ...
    public void onBindViewHolder(ViewHolder holder, int position) {
        ...
        if (mMovies.get(position).getPosterPath() != null) {
            requestCreator = Picasso.with(holder.movieImage.getContext()).load(Uri.parse("http://image.tmdb.org/t/p/w500" + mMovies.get(position).getPosterPath()));
        } else {
